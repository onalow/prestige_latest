...
class CoinPaymentController extends Controller {
    ...
    public function index($serialize) {
      ...
      return view('cabinet.deposit', $data);
    }
    ...
    public function ajax_rates(Request $req, $usd){
      ...
      foreach($rates as $i => $coin){
        if((INT) $coin['is_fiat'] === 0 AND $rates[$i]['rate_btc'] !== 0){
         try {
    ...
    {
      $trx = cointpayment_log_trx::where('status', 100)->first();
      $trx['request_type'] = 'schedule_transaction';
      // dd($trx);
      $trx = $trx->toArray();
      dispatch(new coinPaymentCallbackProccedJob($trx));
    }

    public function getTrxns()
    {
      $trxns = cointpayment_log_trx::where('user_id', auth()->id())->latest()->get();

       return response()->json($trxns);
    }
    ...
    public function createWithdrawal(Withdrawal $withdrawal)
    {
      ...
      try { 
            ...
            if ($result['result']) {
              ...
              $withdrawal->withdrawal_id = $result['result']['id'];
              if ($withdrawal->save()){

                // Mail::to($withdrawal->user)->send(new PaymentSent($withdrawal));
    ...
    try { 
          ...
          if ($payment['error'] == 'ok') {
            ...
            if ($result['status'] == 2) {
              ...
              $withdrawal->pay_status = $result['status_text'];
              $withdrawal->save();
